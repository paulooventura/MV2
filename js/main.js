...
const TMJ_KNOWL_GID = 170;
const TMJ_KNOWL_GIDS = new Set([170, 6685]);
...
const TMJ_SPAWN_MARKER_GIDS = new Set([...TMJ_PLAYER_SPAWN_GIDS,...TMJ_MIND_SPAWN_GIDS,...TMJ_RCA_SPAWN_GIDS,...TMJ_KNOWL_GIDS,...TMJ_ROPE_PICKUP_GIDS,TMJ_MIND_ENEMY_GID,TMJ_ENEMY_CIRCLE_GID,TMJ_ENEMY_SQUARE_GID,TMJ_ENEMY_FLYER_GID,TMJ_ENEMY_BALL_GID,TMJ_ENEMY_SQ_GID,TMJ_ENEMY_TRI_GID,TMJ_ENEMY_STAR_GID,...TMJ_LAITU_GIDS,...TMJ_SIGNOL_GIDS]);
...
function update(){
  ...
  for(const e of ENEMS){
    if(e.mind){
      if(e.alive){if(e.hitF>0)e.hitF--;_updateMindEnemy(e);const playerFeet=p.y+FEET_OFF,fL=p.x+FEET_L,fR=fL+FEET_W;const stompFromAbove=p.vy>1.2&&playerFeet<=e.y+14&&fR>e.x+8&&fL<e.x+e.w-8;if(stompFromAbove&&_shutdownTimer<=0){_damageEnemy(e,ENM_DMG.stomp,p.x+SW/2,p.y+FEET_OFF,0.5);p.vy=-9;e._stompDuck=28;e.crouchAmt=1;e.vy=0;e.og=true;}}
