...
function gridSolidBlocksX(c,r,contactX){
  ...
  const lip=Math.max(28,(typeof WHEEL_R!=='undefined'?WHEEL_R:20)+8);
  for(const dc of [-1,1]){
  ...
  for(const dc of [-1,1]){
    for(const dr of [-3,-2,-1,0,1,2,3]){
      if(!gridIsSlope(c+dc,r+dr)) continue;
...
/** Horizontal-only = walk in front (ghost). Up+dir climbs, down+dir rides down. */
function gridRampIntent(){
  try{
    const h=typeof _moveInputX==='function'?_moveInputX():0;
    const up=typeof isUp==='function'&&isUp();
    const dn=typeof isDn==='function'&&isDn();
    if(h&&up) return 'climb';
    if(h&&dn) return 'drop';
  }catch(e){}
  return 'through';
}
function gridSlopePass(c,r){
  const s=gridSlopeAt(c,r);
  return !!(s&&s.pass);
}
function gridSlopeLive(c,r){
  if(!gridIsSlope(c,r)) return false;
  if(!gridSlopePass(c,r)) return true;
  const want=gridRampIntent();
  return want==='climb'||want==='drop';
}
...
function gridSlopeY(c,r,wx){
  ...
  // Round only the true foot/crest of a run — per-tile fillets make stairs.
  const contHigh=kind==='R'
    ? gridSlopeKind(c+1,r-1)==='R'
    : gridSlopeKind(c-1,r-1)==='L';
  if(contHigh) return y;
  const fillet=Math.min(t*0.28, 10);
  if(kind==='L'){
    const d=wx-c*t;
    if(d>=0&&d<fillet){
...
function gridSet(c,r,type,meta){
  ...
  const pass=!!(meta&&meta.pass);
  if(type===MV_GRID_SLOPE_L){
    MV_GRID.slope[i]={x1:x0,y1:y0,x2:x1,y2:y1,angle:Math.atan2(t,t),pass};
  }else if(type===MV_GRID_SLOPE_R){
    MV_GRID.slope[i]={x1:x0,y1:y1,x2:x1,y2:y0,angle:Math.atan2(-t,t),pass};
  }else{
...
function gridRayHit(x1,y1,x2,y2,opts){
  ...
  const latch=(c,r,tEnter,nx,ny)=>{
    ...
    if(type===MV_GRID_SLOPE_L||type===MV_GRID_SLOPE_R){
      if(!gridSlopeLive(c,r)) return null;
...
/** Stamp a typed grid from world AABBs + 45° slope runs (labs, synthetic tests). */
function buildCollisionGridFromWorld(ww, wh, tile, spec){
  tile=tile||32;
  spec=spec||{};
  const cols=Math.max(8, Math.ceil(ww/tile)|0);
  const rows=Math.max(8, Math.ceil(wh/tile)|0);
  const grid=[];
  for(let r=0;r<rows;r++){
    grid[r]=new Array(cols);
    for(let c=0;c<cols;c++) grid[r][c]=MV_GRID_AIR;
  }
  MV_GRID={ready:true, cols, rows, tile, tw:tile, th:tile, sc:1, data:grid, slope:{}};
  const stampRect=(x,y,w,h,type)=>{
    const c0=Math.floor(x/tile), c1=Math.floor((x+Math.max(1,w)-1)/tile);
    const r0=Math.floor(y/tile), r1=Math.floor((y+Math.max(1,h)-1)/tile);
    for(let r=r0;r<=r1;r++) for(let c=c0;c<=c1;c++){
      ...
      grid[r][c]=type;
      if(MV_GRID.slope) delete MV_GRID.slope[r*cols+c];
    }
  };
  for(const s of spec.solids||[]) stampRect(s.x, s.y, s.w, s.h, MV_GRID_SOLID);
  for(const sl of spec.slopes||[]){
    const kind=sl.kind==='L'?'L':'R';
    const n=Math.max(1, sl.n|0);
    for(let i=0;i<n;i++){
      const c=sl.c+i;
      const r=kind==='R'?sl.r-i:sl.r+i;
      if(c<0||r<0||c>=cols||r>=rows) continue;
      gridSet(c, r, kind==='L'?MV_GRID_SLOPE_L:MV_GRID_SLOPE_R, {pass:!!sl.pass});
      if(sl.pass) continue;
      for(let rr=r+1;rr<rows;rr++){
        const t=grid[rr][c];
        if(t===MV_GRID_SLOPE_L||t===MV_GRID_SLOPE_R) break;
        if(t===MV_GRID_AIR) grid[rr][c]=MV_GRID_SOLID;
      }
    }
  }
  return MV_GRID;
...
/** Headless: walk a wheel up SLOPE_R onto a matching flat. Restores MV_GRID. */
function gridSelftestSlopeCorner(){
  const saved=MV_GRID;
  try{
    const tile=32, sw=64, feetOff=88;
    const floorY=tile*10;
    const n=6, c0=1, r0=9;
    const crestY=(r0-(n-1))*tile;
    const platX=(c0+n)*tile;
    buildCollisionGridFromWorld(tile*20, tile*12, tile, {
      solids:[
        {x:0, y:floorY, w:tile*20, h:tile*2},
        {x:platX, y:crestY, w:tile*6, h:floorY-crestY+tile}
      ],
      slopes:[{c:c0, r:r0, n:n, kind:'R'}]
    });
    const walk=(vx, steps)=>{
      const pl={x:c0*tile+tile*0.5-sw*0.5, y:floorY-feetOff, vx:vx, vy:0, og:true, crouchAmt:0, _grindF:0, _slopeAngle:Math.atan2(-tile,tile)};
      for(let i=0;i<steps;i++){
        pl.vx=vx;
        if(typeof gridResolvePlayer==='function') gridResolvePlayer(pl, vx, 0, {});
      }
      return pl;
    };
    const fast=walk(3.2, 140);
    const slow=walk(0.7, 520);
...
function gridColumnSurface(col, contactX, feetY, maxUp){
  ...
  for(let r=r0;r<=r1;r++){
    let y=null, a=0;
    if(gridSlopeLive(col,r)){
      y=gridSlopeY(col,r,contactX);
      const s=gridSlopeAt(col,r);
      a=s?s.angle:0;
    }else if(gridSolid(col,r) && !_gridIsRampFill(col,r,contactX,feetY)){
      y=r*t;
    }else continue;
...
/** Standable surface in a column near the feet (a few px above or below). */
function gridColumnStandNear(col, contactX, feetY, band){
  if(!_gridReady()) return null;
  band=band==null?14:band;
  const t=MV_GRID.tile;
  const r0=Math.floor((feetY-band)/t), r1=Math.floor((feetY+band)/t);
  let best=null, ang=0, bestAbs=1e9;
  for(let r=r0;r<=r1;r++){
    let y=null, a=0;
    if(gridSlopeLive(col,r)){
      y=gridSlopeY(col,r,contactX);
    ...
    if(y==null||Math.abs(y-feetY)>band) continue;
    const d=Math.abs(y-feetY);
    if(d<bestAbs){ bestAbs=d; best=y; ang=a; }
  }
  return best==null?null:{y:best, ang};
}
...
function _gridSlideOnto(body, col, contactX){
  const feet=body.feetY!=null?body.feetY:body.y+body.h;
  const surf=gridColumnStandNear(col, contactX, feet, 14);
  if(!surf) return false;
  const prevY=body.y, prevFeet=body.feetY, prevAng=body.slopeAng, prevOg=body.onGround;
  gridSetFeet(body, surf.y, surf.ang||0);
  if(_gridBlockedAt(body, col, contactX)||_gridFeetWouldClip(body, body.feetY)){
    body.y=prevY; body.feetY=prevFeet; body.slopeAng=prevAng; body.onGround=prevOg;
    return false;
  }
  return true;
}
...
function _gridStepUp(body, col, contactX){
  ...
  const feet=body.feetY!=null?body.feetY:body.y+body.h;
  // On a ramp, a same-height cube is the height map — not a kerb to hop.
  if(Math.abs(body.slopeAng||0)>0.05){
    const lip=gridColumnSurface(col, contactX, feet, MV_GRID_STEP_UP);
    if(lip && Math.abs(lip.y-feet)<=MV_GRID_STEP_UP){
      gridFollowSlope(body, true);
      if(!_gridBlockedAt(body, col, contactX)) return true;
    }
  }
  const surf=gridColumnSurface(col, contactX, feet, MV_GRID_STEP_UP);
  if(!surf) return _gridSlideOnto(body, col, contactX);
  const rise=feet-surf.y;
  if(rise<=0.01||rise>MV_GRID_STEP_UP) return _gridSlideOnto(body, col, contactX);
...
function gridMoveX(body, dx){
  ...
  if(_gridBlockedAt(body, col, contactX)){
    ...
    if(_gridSlideOnto(body, col, contactX)){
      return;
    }
...
function gridBestFloor(body, prevFeet){
  ...
  // Center-first like Sonic/Mega Man X sensors. Side samples are fallback only.
  const samples=[wx, wx-wr+6, wx+wr-6];
  let best=null, bestAng=0, bestKind=null;
  ...
  for(const sx of samples){
    ...
    for(let row=row0-2;row<=row0+3;row++){
      ...
      if(type===MV_GRID_SOLID||type===MV_GRID_DESTRUCT||type===MV_GRID_ONEWAY){
        if(_gridIsRampFill(c,row,sx,feet)) continue;
        const top=row*t;
        if(prevFeet<=top+0.001 && feet>=top-0.5){
          if(best==null||(bestKind!=='slope'&&top<best)){ best=top; bestAng=0; bestKind='solid'; }
        }
      }else if(gridSlopeLive(c,row)){
        const sy=gridSlopeY(c,row,sx);
        if(sy==null) continue;
        if(prevFeet<=sy+t && feet>=sy-4 && feet<=sy+t){
          if(best==null||bestKind!=='slope'||sy<best){
            best=sy;
    ...
    if(bestKind==='slope') break;
  }
  return best==null?null:{y:best, ang:bestAng};
}

/** Seat the wheel on the height map under its center — not the highest of three stair samples. */
function gridFollowSlope(body, wasGrounded){
  ...
  const wx=body.x+body.w*0.5;
  const wr=body.wheelR||20;
  const feetPre=body.feetY!=null?body.feetY:body.y+body.h;
...
function _gridIsRampFill(c,r,wx,feetY){
  ...
  for(let dc=-1;dc<=1;dc++){
    for(let sr=r-3;sr<=r+1;sr++){
      ...
      if(sy==null) continue;
      if(feetY<=sy+8 && r*t>sy+1) return true;
    }
  }
  return false;
}
...
function gridWheelWedge(body){
  ...
  for(let c=c0;c<=c1;c++) for(let r=r0;r<=r1;r++){
    ...
    if(_gridIsRampFill(c,r,cx,feet)) continue;
    if(Math.abs(r*t-feet)<10) continue;
...
function gridStandBest(cx, feetY, opts){
  ...
  const mid=gridStandHit(cx, feetY, opts);
  if(mid) return mid;
  let best=null, bestAbs=1e9;
  for(const x of [cx-span, cx+span]){
    const hit=gridStandHit(x, feetY, opts);
    if(!hit) continue;
    const d=Math.abs(hit.y-feetY);
    if(d<bestAbs){ bestAbs=d; best=hit; }
  }
  return best;
}
